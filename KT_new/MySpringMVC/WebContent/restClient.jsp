...
// 			var result = confirm(name+"님을 정말 삭제하시겠습니까?");
// 			if(result){
// 				$.ajax({
// 					url:'users/'+id,
// 					method:'DELETE',
	...
	function user_list_result(xhr) {
		...
		$.each(xhr, function(idx, user) {
			...
			$("<tr>")
				...
				.append($("<td>").html("<button class='btn-danger' id='btnDelete'>삭제</button>"))
				.append($("<input type='hidden' id='hidden_id'>").html(user.id))
				.append($("<input type='hidden' id='hidden_name'>").html(user.name))
				.appendTo('tbody');
		});
	}
